...
mod test {
    ...
    #[test]
    fn append_apply_append() -> eyre::Result<()> {
        let tree_depth = 30;
        let empty_value = Hash::ZERO;

        let mut cascading_tree: CascadingMerkleTree<PoseidonHash, _> =
            CascadingMerkleTree::new(vec![], tree_depth, &empty_value);
        let mut identity_tree = IdentityTree::new(tree_depth);

        assert_eq!(cascading_tree.root(), identity_tree.tree.root());

        let batch_size = 10;
        let num_batches = 10;

        let leaves: Vec<Hash> = (0..batch_size * num_batches)
            .map(|i| Hash::from(i as u64))
            .collect();

        let batches: Vec<Vec<Hash>> =
            leaves.chunks(batch_size).map(|x| x.to_vec()).collect();

        let append_batch = |identity_tree: &mut IdentityTree<Vec<Hash>>,
                            batch_idx: usize| {
            let leaf_updates = batches[batch_idx]
                .iter()
                .enumerate()
                .map(|(idx, value)| {
            ...
            identity_tree
                .append_updates(identity_tree.latest_root(), leaf_updates)
                .unwrap()
        };

        cascading_tree.extend_from_slice(&batches[0]);

        let batch_0_post_root = cascading_tree.root();

        cascading_tree.extend_from_slice(&batches[1]);

        let id_tree_root_after_batch_0 = append_batch(&mut identity_tree, 0);

        assert_eq!(id_tree_root_after_batch_0, batch_0_post_root);

        identity_tree.apply_updates_to_root(&batch_0_post_root);

        let id_tree_root_after_batch_1 = append_batch(&mut identity_tree, 1);

        assert_eq!(
            cascading_tree.root(),
            identity_tree.latest_root(),
            "Latest root mismatch"
        );
        assert_eq!(
            cascading_tree.root(),
            id_tree_root_after_batch_1,
            "Append root mismatch"
        );
